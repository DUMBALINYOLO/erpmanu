...
import { getShiftSchedules} from '..//../actions/shiftschedules';
import "./form.css";


class ShiftSchedules extends Component {
    ...
    static propTypes = {
        shiftschedules : PropTypes.array.isRequired,
        getShiftSchedules: PropTypes.func.isRequired,

    };

    componentDidMount() {
        this.props.getShiftSchedules();
    }

    renderHeader() {
        return (
            <div className="table-head">
                <h1>List of Shift Schedules</h1>
    ...
    render() {
        ...
        return (
            <div className="datatable-doc-demo">
                <DataTable ref={(el) => this.dt = el} value={this.props.shiftschedules}
                    style={{backgroundColor: '#4EB08E'}}
                    header={header} responsive className="table-head" dataKey="id" rowHover globalFilter={this.state.globalFilter}
                    selection={this.state.selectedShiftSchedules} onSelectionChange={e => this.setState({selectedShiftSchedules: e.value})}
                    paginator rows={10} emptyMessage="No ShiftSchedules found" currentPageReportTemplate="Showing {first} to {last} of {totalRecords} entries"
...
const mapStateToProps = state =>({
    shiftschedules: state.shiftschedules.shiftschedules
})

export default connect(mapStateToProps, {getShiftSchedules} ) (ShiftSchedules);
